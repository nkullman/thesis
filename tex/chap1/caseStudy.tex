...
Finally, we note that the general lack of conflict between objectives and the similarity across climate scenarios makes this case study a rigorous test of the utility of the proposed conflict metric and the EMO measures. The hypervolumes are all relatively large -- over 80\% of the objective space is occupied by solutions in all climate scenarios. In addition, in all but one pairwise objective comparison, it is difficult to discern any distinct conflict relationship between the objectives. Should the difference between scenarios have been more pronounced, or if the objectives had been in greater conflict with one another, the utility of the process we demonstrate here would only increase.
%
...
%THIS WAS A GOOD CASE STUDY, BECAUSE THE CONFLICT BETWEEN OBJS AND SCENARIOS WAS NOT THAT DIFF, SO IT TESTED THE METRIC'S ABILITY TO SNIFF OUT SMALL DIFFS. IN OTHER CASES, IF THE FRONTIERS VARY MUCH MORE, ITS UTILITY WILL ONLY IMPROVE.