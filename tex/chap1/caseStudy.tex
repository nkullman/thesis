...
With increasing climate change severity, we noted an increase in the number of solutions generated by our model:  51 for $Z_{\text{None}}$, 701 for $Z_{\text{E45}}$, and 1083 for $Z_{\text{E85}}$. We speculate that this is due to the same cause as the increase in the range of NSO habitat provided -- the impact of fuel removals on whether a stand qualifies as NSO habitat.
...
Together, these factors lead to an increase in the number of solution as well as greater diversity in the total amount of NSO habitat provided by the models.
...
% Continues here
From the hypervolumes alone, it is uncertain whether None represents a strictly better frontier than either E45 or E85 or if, despite their smaller hypervolume values, E45 and E85 enclose some region of the objective space that is not enclosed by None. Any such region extends further into the objective space, which represents the presence of solutions that achieve greater joint provision of ecosystem services. The results of the binary hypervolume were presented in Table \ref{tab:binaryHypervols}.

These results show that no frontier is dominated by any other, and each frontier encloses some region of the objective space not enclosed by the others. For the pairs of frontiers for which the binary hypervolume is greatest ($(Z_\text{None},Z_\text{E85})$ and $(Z_\text{E45},Z_\text{E85})$), this additional extension into the objective space is most obvious in Figure \ref{fig:pairplotSedFire}. We see (those areas where they stick out more).

These areas are also the cause of the lessened conflict between Sed-Fire in E45 and None than in E85. We note the success of the conflict metric $C_{ij}$ here: all frontier cross-sections in this plane are very similar -- approachnig the 2D ideal in all situations and having very similar shapes, yet the metric is able to distinguish difference in conflict between them.


For the other pairwise objective comparisons, we saw in Figures pairplot1 and pairplot2 that the distribution of solutions had the appearance of being nearly uniformly scattered. There was no clear conflict pattern between them. The conflict metric reported little conflict between these objective pairs, however it varied in unexpected ways between climate scenarios. We find that the cijd component was primarily responsible for these variations, as the rank correlations tended to be insignificantly different from 0.5. Thus, we find that the smaller one component of the conflict metric, the more sensitive it is to the other.
