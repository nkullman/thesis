...
In the definitions that follow, the following notation is used: the set of solutions $x \in X$ to the set of $m$ objective functions, $F = \{f_i(x), i \in {1,\ldots,m}\}$ may also be written as the set of $m$-dimensional \textbf{objective vectors} $\mathbf{z} \in Z$, where for each solution $x \in X$, the corresponding objective vector is $\mathbf{z} = [f_1(x),f_2(x),\ldots,f_m(x)] \equiv [z^1, z^2,\ldots,z^m]$. In this work, it is assumed, without loss of generality, that all objectives are maximized.

\paragraph{Trade-offs}
The trade-off between two objective vectors is defined informally as the exchange in objective provisions between them. Formally, the trade-off between two objective vectors $\mathbf{a}$ and $\mathbf{b}$ is defined as the vector
\begin{align}
