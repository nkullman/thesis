...
\paragraph{Multi-objective problem and solutions}
...
\end{align}
with \textit{objective functions} $f_i(\mathbf{x}), i \in \{1,\ldots,M\}$ and feasible \textit{decision vectors} (or \textit{solutions}) $\mathbf{x} \in \mathbb{R}^n$. $n$ is the number of decision variables in the optimization problem. A set of equality and inequality constraints determine the \textit{feasible decision vector space} $X$. Each objective function $f_i : \mathbb{R}^n \mapsto \mathbb{R}$ maps decision vectors to scalars in $\mathbb{R}$. The vector objective function $\mathbf{f} : X \mapsto \mathbb{R}^M$ maps the feasible decision vector space to the \textit{objective space} $\mathbb{R}^M$.

I define $\mathcal{M}$ as the set of all $M$ objective functions $\mathcal{M} = \{f_1,\ldots,f_m\}$. $\mathcal{L} \subset \mathcal{M}$ defines a sub-dimensional objective set that includes only some of the objectives $f_i \in \mathcal{M}$.
...
\paragraph{Trade-offs} HERE
The trade-off between two objective vectors is the exchange in objective provisions between them. Formally, the trade-off between two objective vectors $\mathbf{a}$ and $\mathbf{b}$ is defined as the vector
\begin{align}
\mathbf{t}_{\mathbf{a},\mathbf{b}} = [b^1 - a^1, b^2 - a^2, \ldots, b^m - a^m]
\end{align}
Note that $\mathbf{t}_{\mathbf{a},\mathbf{b}} = - \mathbf{t}_{\mathbf{b},\mathbf{a}}$. Because all solutions are assumed to be Pareto efficient, not all components will be of the same sign.

\paragraph{Presence of conflict}
If no conflict exists among a set of objectives, the objectives are said to be ``stacked'' or nonconflicting. In such a set, increasing the provision of one objective also increases provision of the others. Formally, I determine whether a set of objectives $\mathcal{M} \subset F$ is stacked by checking if the following holds true:
\begin{align}
\forall z_1, z_2 \in Z, i \in \mathcal{M}& \notag \\
&(z_1^i \ge z_2^i) \Rightarrow (z_1^j \ge z_2^j) \qquad \label{eqn:objPairHarmony}\\
\forall j \in \mathcal{M}, j \neq i&
\end{align}
This definition is functionally equivalent to that suggested by Zitzler (2009) \cite{brockhoff2009objective} and Purshouse and Fleming (2003) \cite{purshouse2003conflict}.

If the above holds, then the objectives are stacked. Else, there is conflict among the objectives, and the set $\mathcal{M}$ defines a bundle. Note that the bundle may still be reduced by identifying subsets of $\mathcal{M}$ that are stacked. If the statement \eqref{eqn:objPairHarmony} holds for some pair of objectives $i,j \in \mathcal{M}$, $j \neq i$, then the objectives are stacked, and one may be removed from the set $\mathcal{M}$ in order to define a smaller bundle.

\paragraph{Measure of conflict}
Many authors have previously measured conflict (CITEABUNCHLIKEZITZLERANDPURSHOUSEANDGALLEBERLING). Here, we 
Take all pairs of lines on the PCoords plot. How many of them have same sign on slope vs how many have  diff sign on slope? Zero slope = skip that pair in determining test?
