...
%
%WHAT IS CONFLICT. WHAT ARE TRADEOFFS
%HOW TO QUANTIFY AND MEASURE THESE
...
Given a sub-dimensional objective set $\mathcal{L}$, define the \textit{sub-dimensional trade-off} $\tau^{\mathbf{z}^1,\mathbf{z}^2}_\mathcal{L}$ as the vector with components $\tau^{1,2}_\ell$, $\forall \ell \in \mathcal{L}$.
...
\end{align}
holds, then the objectives are said to \textit{decrease monotonically}. When the objectives represent goods or services, define a set of objectives that conflict as a \textit{bundle} and a set of objectives that do not conflict as a \textit{stack}.

This approach to checking for the presence of conflict among an objective set is functionally equivalent to that used by other studies, such as Brockhoff and Zitzler (2009) \cite{brockhoff2009objective} and Purshouse and Fleming (2003) \cite{purshouse2003conflict}.

\subsection{A new measure of conflict}
After determining that objectives conflict with one another (see equation \eqref{eqn:objPairHarmony}), one may wish to determine the severity of the conflict. Consider the frontiers in Figure \ref{fig:ConflictVariesExample}. The conflict between maximization objectives $i$ and $j$ is less in Frontier A than in Frontier B.
\begin{figure}[ht]
\centering
\includegraphics[width=.6\textwidth]{../images/ConflictVariesExample}
\caption[Example of varying conflict between objectives]{Varying conflict between objectives. The maximization objectives $i$ and $j$ are more in conflict in Frontier A than in Frontier B.}
...
\begin{align}
C_{ij} = (1-\rho_{ij})\frac{\overbar{d}_{ij}}{2 d_{\max,ij}} \label{eqn:defConflict}
\end{align}
...
\begin{enumerate}
\item \textbf{Indifference to non-conflicting relationships}. Per the definition of conflict in equation \eqref{eqn:objPairHarmony}, objectives $i,j$ that increase monotonically are not in conflict. Accordingly, $C_{ij}$ should equal 0 in all such cases. This is true for the new metric, since $1-\rho_{ij} = 0$.
\item \textbf{Consideration of objective achievement}. Recall Figure \ref{fig:ConflictVariesExample} and the intuitive notion that the conflict between objectives $i$ and $j$ is stronger in Frontier B than it is in Frontier A. This notion is guided by the idea that the closer objective vectors are to the sub-dimensional ideal solution on average, the less the conflict between the objectives. While correlation measures do not capture this relationship the proposed metric does. In the extreme case of monotonically decreasing objectives, $\frac{(1-\rho_{ij})}{2} = 1$, so $C_{ij} = \frac{\overbar{d}{ij}}{d_{\max,ij}}$. See Figure \ref{fig:WhyOursIsBetter}.
\end{enumerate}
\begin{figure}[ht]
\centering
\includegraphics[width=.8\textwidth]{../images/WhyOursIsBetter}
\caption[Comparing the proposed conflict metric to others used in multi-objective optimization]{HERE.Varying conflict between objectives. The maximization objectives $i$ and $j$ are more in conflict in Frontier A than in Frontier B.}
