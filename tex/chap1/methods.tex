...
We use the binary hypervolume indicator to determine the volume of the objective space that is bounded by frontier $Z^1$ but not frontier $Z^2$. Values of $I_{H2}(Z^1, Z^2) > 0$ indicate areas of the objective space bound only by $Z^1$, implying the presence of regions of less conflict among the objectives. See Figure \ref{fig:binaryHypervolume}.
...
When comparing multi-objective systems, a decision maker may observe unequal hypervolumes across frontiers. Without another means to measure of conflict, the decision maker cannot determine why the inequality exists. Which objective pairs are causing the differences in system-level conflict? We propose here a new conflict metric created specifically to address this question.
