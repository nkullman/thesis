...
Given an efficient frontier, the decision maker will be interested in the conflict among the objectives. To measure conflict over the entire objective set $\mathcal{M}$, we use the hypervolume indicator and the epsilon indicator. To compare conflict between two frontiers, we use the binary equivalent of each of these metrics: the binary hypervolume indicator and the binary epsilon indicator. We describe each of these metrics here. 

\paragraph{Hypervolume Indicators}
The hypervolume indicator measures the proportional volume of the objective space bounded by the frontier. For each relative objective vector $\overbar{\mathbf{z}^i}$ in the frontier $Z$, define its corresponding hyperrectangle by $r_i$. $r_i$ is the $M$-orthotope with diagonally opposite points the origin and the point defined by the components of the relative objective vector. Then the hypervolume indicator is the volume of the union of the hyperrectangles $r_i$:
\begin{align}
...
\end{align}
where $I_{H1} (Z^1 + Z^2)$ is the unary hypervolume indicator of the frontier that consists of the nondominated points in $\{Z^1 \cup Z^2\}$. See Figure \ref{fig:binaryHypervolume}. The binary hypervolume indicator provides the volume of frontier $Z^1$ that is not contained within frontier $Z^2$. Larger values of $I_{H1}$ correspond to frontiers occupying larger amounts of the objective space. $I_{H1} < 1$ indicates some conflict among the objectives. $I_{H2}(Z^1, Z^2) > I_{H2}(Z^2, Z^1)$ indicates areas of less conflict between objectives in $Z^1$ than in $Z^2$.
