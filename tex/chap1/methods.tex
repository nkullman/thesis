...
\label{sec:caseStudy}
A case study on forest management in the Deschutes National Forest serves to demonstrate the application of the hypervolume indicators and the proposed conflict metric to the analysis of multi-objective systems. The case study involves multiple systems corresponding to different realizations of climate change. For each climate change scenario, we solve a multi-objective model that seeks to minimize fire hazard and sediment delivery while maximizing habitat for the northern spotted owl. We compare conflict and objective achievement under each climate change scenario using the hypervolume indicators and the proposed conflict metric.
