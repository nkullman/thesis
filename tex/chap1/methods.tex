\section{Methods}
\label{sec:methods}
We compare conflict across systems using a case study involving climate change and forest management in the Deschutes National Forest. Prior to describing the case study, we first define the terminology used here.
...
% how to quantify conflict? Well, there are these preexisting ways that work well for full-order conflict. Such as hypervol and epsilon. But often we want to know about a pair of objectives, and no good way exists for that currently.
...
\subsection{Other ways to measure a frontier}
% Besides conflict, here are some other things that might be useful for a decision maker when assessing their potential solutions
