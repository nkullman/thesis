...
\paragraph{Defining trade-offs and conflict}
...
\begin{align}
\mathbf{\tau}_{\mathbf{z}^1,\mathbf{z}^2} = [z^2_1 - z^1_1, z^2_2 - z^1_2, \ldots, z^2_M - z^1_M]
\end{align}
Note that $\mathbf{t}_{\mathbf{a},\mathbf{b}} = - \mathbf{t}_{\mathbf{b},\mathbf{a}}$.

The presence of \textit{conflict} among a set of objectives $\mathcal{L} \subset \mathcal{M}$ is determined by checking if $\forall z^1, z^2 \in Z, i \in \mathcal{L}$
\begin{align}
(z^1_i \ge z^2_i) \Rightarrow (z^1_j \ge z^2_j) \quad \forall j \in \mathcal{L}, j \neq i \label{eqn:objPairHarmony}
\end{align}
If \eqref{eqn:objPairHarmony} holds, then the objectives in the objective set $\mathcal{L}$ are not in conflict, and define a \textit{stack} of objectives. Else, there is conflict among the objectives in $\mathcal{L}$, and the set defines a \textit{bundle}. This means of checking for the presence of conflict among an objective set is functionally equivalent to that suggested by Zitzler (2009) \cite{brockhoff2009objective} and Purshouse and Fleming (2003) \cite{purshouse2003conflict}.

\subsection{A new metric to measure conflict}
After determining that objectives conflict with one another, it may be useful to determine the severity of the conflict. To do so requires a means of measuring this value. Many authors have previously measured conflict between objectives (CITEABUNCHLIKEZITZLERANDPURSHOUSEANDGALLEBERLING). Often times these measurements are intended to determine which objective pairs are negatively correlated, which are positively correlated, and which are independent. Most commonly used metrics derive from measures of linear correlation (such as the Pearson correlation coefficient (CITESOMEPEOPLEWHOHAVEUSEDIT-DEBSAXENA)) or rank correlation (such as Kendall's Tau (CITESOMEMOREPEOPLE), Spearman's rho (MORECITES...), and Zitzler's $\delta$-error (CITEZITZLER'SPIECE)). The intent of these metrics is generally to determine which objectives, out of a large set ($M \ge 4$) may be removed from the problem to make it easier to solve.

I argue, however, that none of these metrics adequately captures the severity of conflict between a pair of objectives.
