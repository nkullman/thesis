...
%To help a decision maker understand the total conflict in the multi-objective system, we employ the hypervolume indicator, which measures the percentage of the objective space bounded by the frontier. The hypervolume indicator maps an efficient frontier to a scalar value representing conflict. However, the scalar value provided by the indicator does not provide the decision maker with detail about where the conflict exists within a multi-objective system. To address this question, we developed a new metric that quantifies the conflict between a pair of objectives, allowing the decision maker to determine from which objective pair does the conflict indicated by the hypervolume indicator originate.

%When a decision maker has more than one multi-objective system to analyze, they require a means of comparing the efficient frontiers. To make this comparison, we use the binary hypervolume indicator. The metric is similar to its non-binary counterpart mentioned above, except it maps two efficient frontiers to a scalar. That is, it provides a value for each pair of frontiers, rather than for each frontier individually.

\subsubsection{Hypervolume indicator}
The \textit{hypervolume indicator} $I_{H1}(Z)$ measures the percentage of the objective space that is bounded by the non-dominated objective vectors $\mathbf{z} \in Z$. See Figure \ref{fig:frontierVolumes}. Systems with less conflict will produce solutions with greater joint provision of objectives, leading to a greater proportion of enclosed objective space and thus a larger value for the hypervolume. In contrast, systems with more conflict produce solutions with less joint provision of objectives, leading to lesser proportion of enclosed objective space and a smaller value for the hypervolume. That is, the greater the value of the hypervolume indicator, the lower the conflict in the system.
...
If a frontier $Z^2$ is found to have a smaller hypervolume than another $Z^1$, how do you determine whether $Z^2$ is completely enclosed within $Z^1$ or simply bounds a different but smaller region of the relative objective space? We use the binary hypervolume indicator to address this question. The binary hypervolume $I_{H2}(Z^1,Z^2)$ computes the volume of the objective space bounded by $Z^1$ but not by $Z^2$. See Figure \ref{fig:binaryHypervolume}. As such, if $Z^2$ is completely enclosed within $Z^1$, then $I_{H2}(Z^2,Z^1) \le 0$. On the other hand, if $I_{H2}(Z^2,Z^1) > 0$ then $Z^2$ encloses regions of the objective space that $Z^1$ does not.

Following the definition proposed by Zitzler (1999) \cite{zitzler1999evolutionary}, we define the \textit{binary hypervolume indicator} of two frontiers $Z^1$ and $Z^2$ as \cite{zitzler1999evolutionary}
\begin{align}
I_{H2} (Z^1,Z^2) = I_{H1} (Z^1 + Z^2) - I_{H1} (Z^2)
\end{align}
where $I_{H1} (Z^1 + Z^2)$ is the unary hypervolume indicator of the frontier that consists of the nondominated points in $\{Z^1 \cup Z^2\}$. See Figure \ref{fig:binaryHypervolume}.
...
Computing they hypervolume is a nontrivial task that has received attention from the EMO community. For a comparison of previous algorithms that compute the hypervolume, see While (2006) \cite{while2006faster}. For this work, we developed our own algorithm to compute the hypervolume indicator.
...
We initialize the algorithm with an empty set of non-dominated solutions in $L$ dimensions; refer to the set as $G$. Let the volume of this nondominated set be denoted $\overbar{V}$. We sequentially add solutions from $Z$ to $G$, at each iteration adding the contribution of the solution $\mathbf{z}$ to the hypervolume indicator $V$. These contributions are computed by multiplying the solution's $m$th component $z_m$ by its contribution to the volume of $G$, $\overbar{V}_\mathbf{z}$. See Figure \ref{fig:AlgoAid} for a visual reference (the solutions' contribution to the volume of $G$, $\overbar{V}_\mathbf{z}$, are the areas of the yellow regions in the figure).
...
When the hypervolume indicates that there is conflict in the system, how does the decision maker determine which objectives are causing the conflict? In the case of the hospital, is it cost and patient throughput that are most conflicting, or is it cost and required back-up? For the forest manager, are carbon sequestration and timber revenue the incompatible objectives responsible for the low hypervolume value, or is it wildlife habitat and timber revenue? If the forest manager oversees multiple independent forests, they may also ask whether the answers to these questions are the same in each. With the current suite of measures available today, the decision maker cannot adequately answer such questions. Here we propose a new measure of conflict to fill this void.
...
\label{sec:caseStudy}
To demonstrate the application of the hypervolume indicators and the proposed conflict metric to the analysis of multi-objective systems, we perform a case study on forest management in the Deschutes National Forest. The case study involves multiple multi-objective systems corresponding to different realizations of climate change. For each climate change scenario, we solve a multi-objective model that seeks to minimize fire hazard and sediment delivery while maximizing habitat for the northern spotted owl. We compare conflict and the provision of objectives under each climate change scenario using the hypervolume indicators and the proposed conflict metric.

\subsubsection{Study system}
\label{subsec:studyArea}
To quantify the impacts of climate change on the conflict among and the joint provision of forest ecosystem services, we employ multi-objective optimization on a study system in the Deschutes National Forest known as the Drink Planning Area. The Drink Area is a 7056 ha area on the east slopes of the Cascade Mountain Range (see Figure \ref{fig:drinkOverview}). Like many forests, the Drink is managed for the simultaneous provision of multiple ecosystem services.  The ecosystem services were selected through a process involving stakeholder meetings and discussions regarding compatible ecosystem services. For the current work, a subset of the chosen ecosystem services was selected for study. While the United States Forest Service (USFS) manages for many services simultaneously, many of the services are stacked rather than bundled, meaning that the ecosystem services are not in conflict. These ecosystem services need not all be considered in the multi-objective optimization model, since the selection and maximization of one ecosystem service entails the maximization of all in the stack. For this reason, we disregard non-conflicting ecosystem services and select a minimal bundle on which to employ the multi-objective optimization. Those that do not conflict can be stacked post-optimization.

The bundle selected for this case study is a set of three ecosystem services that include wildlife habitat, water quality, and fire hazard. The US Forest Service seeks to ensure the sustained provision of these services, but climate change may threaten their ability to do so. Given this set of objectives, a central question to our analysis is, how will climate change impact the joint provision of these services? Will the provision of wildlife habitat require the same sacrifices in fire hazard under different climate conditions? Research agrees that climate change will alter the ability of forests to provide ecosystem services in isolation CITE, but no research has considered the question of how it may impact the conflict among ecosystem services. We do so here by considering three different climate change scenarios and designing a multi-objective model that optimizes ecosystem service achievement under each.
...
The first ecosystem service considered in our model is the provision of habitat for the northern spotted owl (NSO) (\textit{Strix occidentalis caurina}, Figure \ref{fig:nso}). The NSO typically inhabits forested areas on the Pacific coast. It is important for two primary reasons. First, the NSO often serves as an indicator species, meaning that conservation practitioners use the presence and health of the NSO population as a proxy for the overall health of old growth forest habitat (this practice has not been without controversy \cite{simberloff1998flagships}). Second, logging and development along the coast have led to the species being listed as threatened and thus protected under the Endangered Species Act of 1973 \cite{congress1973endangered}. As a result, the USFS is required to protect the species.

Following the convention in Schroder (2016) \cite{schroder2016multi}, we define forested areas that meet the following criteria to be considered suitable NSO habitat:


 Because of the availability of dense old growth forest in the Drink Area, approximately 43\% of the area serves as habitat for the NSO (see Figure \ref{fig:drinkOwlAndWatershed}). The USFS is required to protect this species, because it is listed as threatened and therefore protected by the 
\begin{figure}
\centering
